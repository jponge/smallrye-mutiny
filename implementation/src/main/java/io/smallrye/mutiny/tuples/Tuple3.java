...
public class Tuple3<T1, T2, T3> extends Tuple2<T1, T2> implements Tuple {
    ...
    @Override
    public <T> Tuple3<T, T2, T3> mapItem1(@NotNull Function<T1, T> mapper) {
        return Tuple3.of(mapper.apply(item1), item2, item3);
    }

    @Override
    public <T> Tuple3<T1, T, T3> mapItem2(@NotNull Function<T2, T> mapper) {
        return Tuple3.of(item1, mapper.apply(item2), item3);
    }

    public <T> Tuple3<T1, T2, T> mapItem3(@NotNull Function<T3, T> mapper) {
